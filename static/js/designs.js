...
// For Output
const jsonColored = document.querySelector('#json-colored');
const jsonBinary = document.querySelector('#json-binary');
...
// copypasta from https://stackoverflow.com/questions/1740700/how-to-get-hex-color-value-rather-than-rgb-value
...
function getGridArray(colored = false) {
    return Array.prototype.map.call(document.querySelectorAll('#pixel-canvas tr'), function (tr) {
...
// Output listeners
jsonColored.addEventListener('click', function (e) {
    e.preventDefault();
    var coloredArray = getGridArray(true);
    outputTextarea.value = prettifyArray(coloredArray);
});

jsonBinary.addEventListener('click', function (e) {
    e.preventDefault();
    var binaryArray = getGridArray();
    outputTextarea.value = prettifyArray(binaryArray);
});
